...
try:
    ...
    # DON'T EXIT WHEN IMPORTED BY DJANGO!
    # Only exit if running as main script
    if __name__ == "__main__":
        sys.exit(1)
    else:
        raise  # Re-raise the error for Django to see


class TradingAssistantSystem():
    """Main system class that coordinates all components"""
    # Add this method to TradingAssistantSystem class in main.py
    # Add this method to TradingAssistantSystem class in main.py
    def __init__(self, config_path: Optional[str] = None):
        """
        Initialize the Trading Assistant System
        
        Args:
            config_path: Optional path to configuration JSON file
        """
        self.config = self._load_config(config_path)
        self.assistant = None  # Will be initialized in run() method
    ...
    async def process_single_query(self, query: str) -> Dict:
        ...
        This method is designed for API/web integration.
        ...
        try:
            ...
            # Process the question using SimpleTradingInterface
            response = await self.assistant.process_question(query)
            
            return response
            
        except Exception as e:
            print(f"❌ Error in process_single_query: {e}")
            import traceback
            traceback.print_exc()
            return {
                "success": False,
    ...
    def process_query_sync(self, query: str) -> Dict:
        """
        Synchronous wrapper for process_single_query.
        ...
        Returns:
            Dict with analysis results from orchestrator
        """
        # Create new event loop for this sync call
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            result = loop.run_until_complete(self.process_single_query(query))
            return result
        except Exception as e:
            ...
            return {
                "success": False,
                "error": str(e),
                "message": "Failed to process query synchronously"
            }
        finally:
            loop.close()
